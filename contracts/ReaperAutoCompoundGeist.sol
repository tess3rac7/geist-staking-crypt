...
contract ReaperAutoCompoundGeist is ReaperBaseStrategy {
    ...
    address public constant geistStaking =
        address(0x49c93a95dbcc9A6A4D8f77E59c038ce5020e82f8);
    address public constant geistAddressesProvider =
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (geistBal < _amount) {
            IGeistStaking(geistStaking).withdraw(_amount - geistBal);
        ...
        uint256 withdrawFee = (geistBal * securityFee) / PERCENT_DIVISOR;
        IERC20(geist).safeTransfer(vault, geistBal - withdrawFee);
    }
    ...
    {
        IGeistStaking.RewardData[] memory rewardDataArray = IGeistStaking(
            geistStaking
        ...
        for (uint256 i = 0; i < rewardDataArray.length; i++) {
            ...
            if (
                rewardDataArray[i].token == geist ||
                rewardDataArray[i].amount == 0
            ) {
                continue;
            }

            // for all other reward gTokens, add wFTM equivalent to profit
            address baseToken = IAToken(rewardDataArray[i].token)
                .UNDERLYING_ASSET_ADDRESS();
            if (baseToken == wftm) {
                profit += rewardDataArray[i].amount;
            } else {
    ...
    function claimRewardsAndSwapToWftm() internal {
        ...
        ILendingPool lendingPool = ILendingPool(
            ILendingPoolAddressesProvider(geistAddressesProvider)
                .getLendingPool()
        );

        // 1. claims rewards from the Geist Staking contract
        IGeistStaking.RewardData[] memory rewardDataArray = stakingContract
            .claimableRewards(address(this));
    ...
    function balanceOf() public view override returns (uint256) {
        return balanceOfGeist() + balanceOfPool();
    }
    ...
    function panic() external {
        ...
        pause();
        (uint256 withdrawableBalance, ) = IGeistStaking(geistStaking)
     ...
     */
    function addRewardToken(address _token, address[] calldata _path)
        external
        whenNotPaused
        returns (bool)
    {
    ...
    function giveAllowances() internal {
        ...
        for (uint256 i = 0; i < rewardBaseTokens.length; i++) {
            ...
            IERC20(rewardBaseTokens[i]).safeApprove(
                uniRouter,
                type(uint256).max
            );
        }
